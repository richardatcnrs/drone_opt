...
def run_cqm_and_collect_solutions(model, sampler):
    sampleset = sampler.sample_cqm(model, 5)
    return sampleset
...
def process_solutions(sampleset, n, weight_matrix, replaced_edges):
    ...
    print(perm_solutions)
    return None

def check_perm(solution, n):
    for i in range(1, n):
        if not(sum(solution['x_' + str(i) + '_' + str(j)] for j in range(1,n)) == 1):
            return False

    for j in range(1, n):
        if not(sum(solution['x_' + str(i) + '_' + str(j)] for i in range(1,n)) == 1):
            return False
...
process_solutions(sampleset, n, weight_matrix, replaced_edges)
